...
class Page10Question extends React.Component {
  ...
  componentDidMount = async () => {
    const doc = await axios.get("/api/post-test-1/questions");
    this.setState({ questions: doc.data });
  };

  handleDelete = async (id) => {
    await axios.delete("/api/post-test-1/questions/" + id);
    await this.setState({ question: "" });
    this.componentDidMount();
  };

  handleAdd = async () => {
    await axios.post("/api/post-test-1/questions", {
    ...
    return (
      <Container>
        <div className="jumbotron">
          <h1 style={{ margin: 20 }}>Post-Test Question 1</h1>
          <Button
            variant="contained"
            color="primary"
            style={{ marginLeft: 20 }}
